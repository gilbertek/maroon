class Transformer
  ...
  def self_method_is_interaction?()
    ((defining_role == nil) or (defining_role.name == nil))
  end
  ...
  def self_method_line_no()
    @lines
  end

  def self_method_file_name()
    (@defining_role.file_name or "")
  end
  ...
  def self_method_generate_source(with_backtrace)
  ...
  def self_definitions_generate(context_class)
    impl = ""
    getters = []
