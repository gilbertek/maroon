...
dependencies
   ...
   def self_methods_dependencies() methods.select { |k, v| (not (v.methods == nil)) and (v.methods.length > 0) }.each do |r, role|
  ...
  @role_name = temp____role_name
   ...
   def self_dependency_add(dependent_role_name,method_name) if dependent_role_name and (not (dependent_role_name == role_name)) then
...
if (self_method_definition.length > (index + 1)) then
  ...
  self_dependency_add(name, method_name) unless (name == nil)
end
