...
ctx
   ...
   def self.generate_files_in() @generate_files_in

 end
   def self.generate_files_in=(folder) @generate_files_in = folder

 end
   def self.generate_code=(value) @generate_code = value

 end
   def self.generate_dependency_graph=(value) @generate_dependency_graph = value

 end
   def self.generate_code() (@generate_code or ((not generate_dependency_graph) or generate_files_in))
 end
   def self.generate_dependency_graph() @generate_dependency_graph

 end
   def dependencies() @dependencies

 end
   def generated_class() @generated_class

 end
   def dependencies=(value) @dependencies = value

 end
   def generated_class=(value) @generated_class = value

 end
   def methods() @methods ||= {  }

 end
   def private_interactions() @private_interactions
...
@with_contracts = value
   ...
   def is_definition?(exp) (exp and ((exp[0] == :defn) or (exp[0] == :defs)))
 end
   def role(role_name,&b) file_name, line_no = b.source_location
